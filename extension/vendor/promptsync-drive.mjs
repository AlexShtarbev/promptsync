// ../platform/server/services/device-auth.ts
...
async function requestDeviceCode(clientId, scope, fetchImpl = fetch) {
...
async function pollOnce(clientId, clientSecret, deviceCode, fetchImpl = fetch) {
...
async function pollForToken(clientId, clientSecret, dc, deps = {}) {
  const fetchImpl = deps.fetchImpl ?? fetch;
  const sleep = deps.sleep ?? ((ms) => new Promise((r) => setTimeout(r, ms)));
  const now = deps.now ?? (() => Date.now());
  let intervalMs = (dc.interval || 5) * 1e3;
  ...
  while (now() < deadline) {
    ...
    if (out.status === "slow_down") intervalMs += 5e3;
  }
...
// ../platform/server/services/drive-rest.ts
var DRIVE_API = "https://www.googleapis.com/drive/v3";
function driveRestApi(getToken, opts = {}) {
...
// ../platform/server/services/file-store-core.ts
var MemFileStore = class {
...
// ../platform/server/services/drive-store.ts
var DRIVE_FOLDER_MIME = "application/vnd.google-apps.folder";
var TEXT_EXTS = /* @__PURE__ */ new Set([".md", ".yaml", ".yml", ".json", ".tsv", ".txt"]);
function extOf(name) {
...
async function buildSnapshot(api, rootFolderId, mountPath = "/drive") {
    ...
    await Promise.all(
      children.map(async (child) => {
        ...
        if (child.mimeType === DRIVE_FOLDER_MIME) {
          ...
          store.set(childPath, new Uint8Array());
...
function extname(p) {
  ...
  return i2 > 0 ? n.slice(i2) : "";
}
var path_shim_default = { join, resolve, basename, dirname, extname, sep: "/" };

// ../platform/node_modules/js-yaml/dist/js-yaml.mjs
...
// ../platform/server/services/markdown-parser.ts
...
function parsePromptSections(body) {
  const sections = {};
...
function findImage(dir) {
  ...
  for (const ext of IMAGE_EXTS) {
    const p = path_shim_default.join(dir, `image${ext}`);
  ...
  for (const f of files) {
    if (IMAGE_EXTS.some((ext) => f.toLowerCase().endsWith(ext))) {
      return path_shim_default.join(dir, f);
    }
  }
  return null;
}
function findStartFrame(dir) {
  if (!fileStore().exists(dir)) return null;
  for (const ext of IMAGE_EXTS) {
    const p = path_shim_default.join(dir, `start-frame${ext}`);
    if (fileStore().exists(p)) return p;
  }
  return null;
}
function findOpenartRef(dir, prefix) {
  const filename = prefix ? `${prefix}-openart-ref.json` : "openart-ref.json";
  const p = path_shim_default.join(dir, filename);
...
function findOpenartResourceId(dir, prefix) {
  const filename = prefix ? `${prefix}-openart-ref.json` : "openart-ref.json";
  const p = path_shim_default.join(dir, filename);
  if (!fileStore().exists(p)) return null;
  try {
...
function parseShot(shotDir, defaultAr = "9:16") {
  const code = path_shim_default.basename(shotDir);
  const shotFile = readMdFile(path_shim_default.join(shotDir, "shot.md"));
  if (!shotFile) return null;
  const meta = {
    shot: shotFile.data.shot ?? code,
    setting: shotFile.data.setting ?? "",
    emotion: shotFile.data.emotion ?? "",
    shot_type: shotFile.data.shot_type ?? "",
    camera: shotFile.data.camera ?? "Static",
    duration: shotFile.data.duration ?? "",
    color_mood: shotFile.data.color_mood ?? "",
    status: shotFile.data.status ?? "draft",
    asset_type: shotFile.data.asset_type ?? "still",
    reuses: shotFile.data.reuses ?? null,
    palette_group: shotFile.data.palette_group ?? null,
    risk: shotFile.data.risk ?? "low",
    multi_shot_group: shotFile.data.multi_shot_group ?? null,
    elements: [...shotFile.data.elements ?? []]
  };
  const content = {
  ...
  const mjFileRaw = readMdFile(path_shim_default.join(shotDir, "mj-prompt.md"));
  const nbFileRaw = mjFileRaw ? null : readMdFile(path_shim_default.join(shotDir, "nb-prompt.md"));
  const mjFileSource = mjFileRaw ?? nbFileRaw;
  const mjPrompt = mjFileSource ? {
    meta: {
      shot: mjFileSource.data.shot ?? code,
      model: mjFileSource.data.model ?? "v7",
      style: mjFileSource.data.style ?? "raw",
      ar: mjFileSource.data.ar ?? mjFileSource.data.aspect_ratio ?? defaultAr,
      platform: mjFileSource.data.platform ?? (nbFileRaw ? "nanobanana" : "mj"),
      reference_images: mjFileSource.data.reference_images ?? {}
    },
...
function findVideo(videosDir, code) {
  if (!fileStore().exists(videosDir)) return null;
  for (const ext of VIDEO_EXTS) {
    const p = path_shim_default.join(videosDir, `${code}${ext}`);
    if (fileStore().exists(p)) return p;
  }
  return null;
}
function attachVideoPrompts(shot, videoPromptsDir, defaultAr = "9:16") {
  const klingDir = path_shim_default.join(videoPromptsDir, shot.code);
  if (!fileStore().exists(klingDir)) return;
  const klingFile = readMdFile(path_shim_default.join(klingDir, "kling-prompt.md"));
  if (klingFile) {
    shot.klingPrompt = {
      meta: {
        shot: klingFile.data.shot ?? shot.code,
        motion_scale: klingFile.data.motion_scale ?? 5,
        aspect_ratio: klingFile.data.aspect_ratio ?? defaultAr,
        mode: klingFile.data.mode ?? "standard",
        multi_shot_group: klingFile.data.multi_shot_group ?? null,
        resolution: klingFile.data.resolution ?? void 0,
        start_frame: klingFile.data.start_frame ?? void 0
      },
      body: klingFile.body,
      sections: parsePromptSections(klingFile.body)
    };
  }
  const seedanceFile = readMdFile(path_shim_default.join(klingDir, "seedance-prompt.md"));
  if (seedanceFile) {
    shot.seedancePrompt = {
      meta: {
        shot: seedanceFile.data.shot ?? shot.code,
        aspect_ratio: seedanceFile.data.aspect_ratio ?? defaultAr,
        duration: seedanceFile.data.duration ?? 5,
        mode: seedanceFile.data.mode ?? "i2v",
        character_lock: seedanceFile.data.character_lock ?? null,
        environment_ref: seedanceFile.data.environment_ref ?? null,
        wardrobe_ref: seedanceFile.data.wardrobe_ref ?? null,
        start_frame: seedanceFile.data.start_frame ?? null,
        camerafixed: seedanceFile.data.camerafixed ?? void 0,
        seed: seedanceFile.data.seed ?? null
      },
      body: seedanceFile.body,
      sections: parsePromptSections(seedanceFile.body)
    };
  }
  const nbFile = readMdFile(path_shim_default.join(klingDir, "nanobanana.md"));
  if (nbFile) {
    shot.nanoBanana = {
      meta: {
        shot: nbFile.data.shot ?? shot.code,
        source: nbFile.data.source ?? "generate-new"
      },
      body: nbFile.body,
...
var REF_SECTION_POSITIVE = /\b(reference|sheet|plate|prompt|angle)s?\b/i;
var REF_SECTION_NEGATIVE = /\b(identity block|element description|character lock|canonical|consistency note|visual anchor|transformation note)\b/i;
function isReferenceSectionHeading(heading) {
  return REF_SECTION_POSITIVE.test(heading) && !REF_SECTION_NEGATIVE.test(heading);
}
function parseCharacterViews(body, charSlug, charsDir) {
  const views = [];
  let autoIndex = 1;
  ...
  for (let h = 0; h < h2Matches.length; h++) {
    ...
    for (const part of subParts) {
      ...
      const prompt = fenceMatch ? fenceMatch[1].trim() : rawPrompt.split("\n").filter((line) => !line.startsWith(">")).join("\n").trim();
      if (!prompt) continue;
      let index;
      let name;
      ...
      for (const ext of IMAGE_EXTS) {
        const p = path_shim_default.join(charsDir, `${imgBase}${ext}`);
        if (fileStore().exists(p)) {
          imagePath = p;
          break;
        }
      }
...
function parseCharacter(filePath, scope = "local") {
  const file = readMdFile(filePath);
  if (!file) return null;
  const meta = {
    name: file.data.name ?? path_shim_default.basename(filePath, ".md"),
    element_name: file.data.element_name ?? "",
    element_type: file.data.element_type ?? "character",
    appears_in: [...file.data.appears_in ?? []],
    status: file.data.status ?? "draft",
    element_status: file.data.element_status ?? "needs-reference",
    parent_environment: file.data.parent_environment ?? null,
    // Location is authoritative for scope; canon links the visual sheet to its bible entry.
    scope,
    canon: file.data.canon ?? null,
    // Optional typed physics specs (physics-engine); importer falls back to Identity-Block prose.
    build: file.data.build ?? null,
    concealment: file.data.concealment ?? null,
    identity_clause: file.data.identity_clause ?? null
  };
  const charSlug = path_shim_default.basename(filePath, ".md");
  const charsDir = path_shim_default.dirname(filePath);
...
function collectElements(sources) {
  const order = [];
  const byKey = /* @__PURE__ */ new Map();
  const fileBaseNames = /* @__PURE__ */ new Map();
  for (const { dir, scope } of sources) {
    ...
    for (const f of mdFiles) {
      const char = parseCharacter(path_shim_default.join(dir, f), scope);
      if (!char) continue;
      ...
      byKey.set(key, char);
      fileBaseNames.set(char, path_shim_default.basename(f, ".md"));
    }
  }
  return { elements: order, fileBaseNames };
}
function elementSources(projectDir, globalElementDirs) {
  return [
    ...globalElementDirs.map((dir) => ({ dir, scope: "global" })),
    ...["characters", "environments", "props"].map((d) => ({
      dir: path_shim_default.join(projectDir, "storyboard", d),
      scope: "local"
    }))
  ];
}
function loadProject(projectDir, globalElementDirs = [], seriesDefaults) {
  const yamlPath = path_shim_default.join(projectDir, "project.yaml");
  if (!fileStore().exists(yamlPath)) return null;
  const rawConfig = jsYaml.load(fileStore().readText(yamlPath));
  const config = {
    name: rawConfig.name ?? "",
    slug: rawConfig.slug ?? "",
    created: rawConfig.created ?? "",
    status: rawConfig.status ?? "",
    drive_folder_id: rawConfig.drive_folder_id ?? null,
    default_style: rawConfig.default_style ?? "",
    shot_prefix: rawConfig.shot_prefix ?? "",
    aspect_ratio: rawConfig.aspect_ratio ?? seriesDefaults?.aspect_ratio ?? "9:16",
    default_resolution: rawConfig.default_resolution ?? seriesDefaults?.default_resolution ?? "1K"
  };
  const shotsDir = path_shim_default.join(projectDir, "storyboard", "shots");
  const videoPromptsDir = path_shim_default.join(projectDir, "storyboard", "video-prompts");
  const videosDir = path_shim_default.join(projectDir, "storyboard", "videos");
  const shots = [];
  if (fileStore().exists(shotsDir)) {
    const shotDirs = fileStore().readDir(shotsDir).filter((d) => d.isDirectory).map((d) => path_shim_default.join(shotsDir, d.name)).sort();
  ...
  const parentEnvSlugs = new Set(
    allElements.map((c) => c.meta.parent_environment).filter(Boolean)
  );
...
function resolveElements(shot, projectDir, globalElementDirs = []) {
...
function loadSingleShot(projectDir, code, globalElementDirs = [], seriesDefaults) {
  const ar = readProjectAspectRatio(projectDir, seriesDefaults);
  const shotDir = path_shim_default.join(projectDir, "storyboard", "shots", code);
  const shot = parseShot(shotDir, ar);
  if (!shot) return null;
  attachVideoPrompts(shot, path_shim_default.join(projectDir, "storyboard", "video-prompts"), ar);
  shot.videoPath = findVideo(path_shim_default.join(projectDir, "storyboard", "videos"), shot.code);
  resolveElements(shot, projectDir, globalElementDirs);
  return shot;
}
function readProjectAspectRatio(projectDir, seriesDefaults) {
  const fallback = seriesDefaults?.aspect_ratio ?? "9:16";
  const yamlPath = path_shim_default.join(projectDir, "project.yaml");
  if (!fileStore().exists(yamlPath)) return fallback;
  try {
    const raw = jsYaml.load(fileStore().readText(yamlPath));
    return raw.aspect_ratio ?? fallback;
  } catch {
    return fallback;
  }
}
function loadSingleCharacter(projectDir, charSlug, globalElementDirs = []) {
  const localDirs = ["characters", "environments", "props"].map(
    (d) => path_shim_default.join(projectDir, "storyboard", d)
  );
  for (const [dir, scope] of [
    ...localDirs.map((d) => [d, "local"]),
    ...globalElementDirs.map((d) => [d, "global"])
  ]) {
    if (!fileStore().exists(dir)) continue;
...
var DISCOVERY_SKIP_DIRS = /* @__PURE__ */ new Set([
  "node_modules",
  "dist",
  "build",
  ".next",
...
function readProjectConfig(yamlPath) {
  try {
    const raw = jsYaml.load(fileStore().readText(yamlPath));
    return {
      name: raw.name ?? "",
      ...
      created: raw.created ?? "",
      status: raw.status ?? "",
      drive_folder_id: raw.drive_folder_id ?? null,
      default_style: raw.default_style ?? "",
      shot_prefix: raw.shot_prefix ?? "",
      aspect_ratio: raw.aspect_ratio ?? "9:16",
      default_resolution: raw.default_resolution ?? "1K"
    };
  } catch {
    return null;
  }
}
function globalElementDirsFor(globalDir) {
  return ["characters", "environments", "props"].map((d) => path_shim_default.join(globalDir, d));
}
function loadSeries(seriesDir, seen) {
  let raw;
  try {
    raw = jsYaml.load(fileStore().readText(path_shim_default.join(seriesDir, "series.yaml")));
  } catch {
    return null;
  }
  const config = {
    name: raw.name ?? path_shim_default.basename(seriesDir),
    slug: raw.slug ?? path_shim_default.basename(seriesDir),
    type: "series",
    status: raw.status ?? "",
    aspect_ratio: raw.aspect_ratio ?? "9:16",
    default_resolution: raw.default_resolution ?? "1K",
    drive_folder_id: raw.drive_folder_id ?? null,
    global_elements: raw.global_elements ?? "storyboard",
    bible: raw.bible ?? "bible"
  };
  const globalDir = path_shim_default.resolve(seriesDir, config.global_elements);
  const globalElementDirs = globalElementDirsFor(globalDir);
  let episodeDirs;
  if (Array.isArray(raw.episodes) && raw.episodes.length) {
    episodeDirs = raw.episodes.map((e) => path_shim_default.resolve(seriesDir, e));
  } else {
    const episodesRoot = path_shim_default.join(seriesDir, "episodes");
    episodeDirs = fileStore().exists(episodesRoot) ? fileStore().readDir(episodesRoot).filter((d) => d.isDirectory).map((d) => path_shim_default.join(episodesRoot, d.name)).sort() : [];
  }
  const episodes = [];
  for (const epDir of episodeDirs) {
    const epYaml = path_shim_default.join(epDir, "project.yaml");
    if (!fileStore().exists(epYaml)) continue;
    const resolved = path_shim_default.resolve(epDir);
    if (seen.paths.has(resolved)) continue;
    const epConfig = readProjectConfig(epYaml);
    if (!epConfig) continue;
    const slug = epConfig.slug || path_shim_default.basename(epDir);
...
var wsCache = null;
var WS_TTL_MS = 1500;
function discoverWorkspace(baseDirs, maxDepth = 4) {
  ...
  const projects = [];
  const series = [];
  const seen = { paths: /* @__PURE__ */ new Set(), slugs: /* @__PURE__ */ new Set() };
  const seenSeries = /* @__PURE__ */ new Set();
  const visit = (dir, depth) => {
    if (fileStore().exists(path_shim_default.join(dir, "series.yaml"))) {
      const resolved = path_shim_default.resolve(dir);
    ...
    const projYaml = path_shim_default.join(dir, "project.yaml");
    if (fileStore().exists(projYaml)) {
      const resolved = path_shim_default.resolve(dir);
      if (seen.paths.has(resolved)) return;
      const config = readProjectConfig(projYaml);
      if (config) {
        const slug = config.slug || path_shim_default.basename(dir);
...
function buildExtensionCharacter(c) {
  return {
    name: c.name,
...
function buildElementMap(characters) {
  const elementMap = {};
...
function buildExtensionIndex(index, slug) {
  return {
    project: slug,
